...
const mockEmergencies: EmergencyRequest[] = [
  {
    ...
    location: { type: 'Point', coordinates: [77.4126, 23.2599] },
    raw_message: null,
    advisory: 'Stay calm. Apply pressure to any bleeding wounds. Keep the person warm and comfortable until help arrives.',
    status: 'open',
...
export class MockApiClient {
  ...
  async createEmergency(payload: CreateEmergencyPayload): Promise<ApiResponse<EmergencyRequest>> {
    ...
    const emergency: EmergencyRequest = {
      ...
      raw_message: null,
      advisory: 'Stay calm and follow the instructions provided by emergency responders. Help is on the way.',
      status: 'open',
      assigned_volunteer: null,
      current_radius_km: 5.0,
      created_at: new Date().toISOString(),
